...
<div class="ui stackable grid">
  <div class="ui row">
    ...
    <div class="ui {if count($WIDGETS_LEFT) && count($WIDGETS_RIGHT) }four wide tablet eight wide computer{elseif count($WIDGETS_LEFT) || count($WIDGETS_RIGHT)}ten wide tablet twelve wide computer{else}sixteen wide{/if} column">
	  ...
	  <div class="ui segment card-body">
		...
		<table id="example" class="ui table dataTables-users" style="width:100%">
		  <thead>
			<tr>
			  <th style="background: var(--bg2-color);color: #ababab;">{$USERNAME}</th>
			  <th style="background: var(--bg2-color);color: #ababab;">{$GROUP}</th>
			  <th style="background: var(--bg2-color);color: #ababab;">{$CREATED}</th>
			</tr>
		  </thead>
		  <tbody>
